...
function yeoman(argv, options) {
  ...
  ['marionette', 'angular', 'todos']
    .forEach(function (name) {
      env.registerStub(require('./generators/' + name), name);
    });
  // Special alias for `bacn create`.
  env.registerStub(require('./generators/todos'), 'default');
...
function loopback(argv, options) {
  ...
  var deferred = when.defer();
  var dir = options.cwd || process.cwd();
  ...
  switch (argv[0]) {
    case 'project':
      ...
      dir = path.resolve(options.cwd || process.cwd(), argv[1]);

      // TODO(schoon) - Get loopback to support a different `name` property.
      workspace.models.Project.createFromTemplate(
        dir,
    ...
    case 'model':
      if (argv.length < 2) {
        return error('lb-shim: Name missing.');
      }

      workspace.models.Project.loadFromFiles(dir, function (err, project) {
        if (err) {
          return deferred.reject(err);
        }
      ...
      break;
    default:
      return error('lb-shim: `lb ' + argv[0] + '` has not been shimmed.');
  }
